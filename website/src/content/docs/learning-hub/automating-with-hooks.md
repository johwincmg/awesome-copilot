...
> **Exit code 2 — soft deny** (v1.0.70+): When a `preToolUse` hook exits with code `2` specifically, the tool call is denied without surfacing an error to the user. This is a "soft deny" — the agent sees the denial as a policy decision rather than a script failure. Exit with `1` (or any other non-zero code) to deny with an error message. Use exit code `2` for routine policy blocks where you don't want alarming error output cluttering the session.
